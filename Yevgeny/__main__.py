...
async def main(sprinter):
    ...
    while True:
        ...
        await asyncio.gather(*[go_through_dialog(p) for p in clients])
        await asyncio.sleep(0.4)
        await asyncio.gather(*[actually_collecting_wisps(p) for p in clients])
        await asyncio.gather(*[exit_out(p) for p in clients])
        await asyncio.gather(*[p.wait_for_zone_change() for p in clients])
        await asyncio.gather(*[p.send_key(Keycode.PAGE_DOWN, 0.1) for p in clients])
